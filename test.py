...
filepath = (r"C:\\Users\\soufi\\Documents\\repositories\\Destination-Insights-with-Google\\google-insights.xlsx")
...
listcountries = [{"name":"Spain","id":"ES"}]

try: 
    # ACCEPT COOKIES
    #driver.find_elements_by_xpath('/html/body/div[1]/div/span[2]/a[2]')[0].click()
    driver.find_element(By.XPATH, '/html/body/div[1]/div/span[2]/a[2]').click()

    # TRIP TYPE      
    driver.find_element_by_id("select_22").click()
    driver.execute_script('document.getElementById("select_option_44").firstChild.click()')
    webdriver.ActionChains(driver).send_keys(Keys.ESCAPE).perform()

    # DEMAND CATEGORY
    driver.find_element_by_id("select_25").click()
    driver.execute_script('document.getElementById("select_option_45").firstChild.click()')
    time.sleep(1)
    driver.execute_script('document.getElementById("select_option_46").firstChild.click()')
    webdriver.ActionChains(driver).send_keys(Keys.ESCAPE).perform()


    # SUBMIT FILTER
    #driver.find_element(By.XPATH, '/html/body/div[1]/div[6]/div/div/div[2]/div[2]/button').click()


    counter = 1 
    listtotal = [] 
    for iteration in listcountries: 
        print("Scraping results for " + iteration["name"]) 
        time.sleep(5) 
        input = driver.find_element_by_xpath('/html/body/div[1]/div[6]/div/div/div[1]/div[2]/md-content[1]/md-select').click() 
        time.sleep(5) 
        choice = driver.find_element_by_xpath('/html/body/div[9]/md-select-menu/md-content/md-option[' + str(counter) + ']').click()  
        time.sleep(5) 
        # Submit
        driver.find_element(By.XPATH, '/html/body/div[1]/div[6]/div/div/div[2]/div[2]/button').click()
        time.sleep(5) 
        #table1 = driver.find_element_by_xpath("/html/body/div[1]/div[10]/div[2]/div[4]") 
        #table2 = driver.find_element_by_xpath("/html/body/div[1]/div[13]/div[2]/div[4]") 
        #table3 = driver.find_element_by_xpath("/html/body/div[1]/div[17]/div[2]") 
        ...
        mydivsregions = table1soup.find_all("span", {"class": "geographic-demand__name ng-binding"}) 
        mydivsregionsnumber = table1soup.find_all("span", {"class": "geographic-demand__total glue-small-text ng-binding"}) 
        ...
        mydivsregionssources = table3soup.find_all("span", {"class": "sources__name ng-binding"}) 
        mydivsregionssourcesnumbers = table3soup.find_all("span", {"class": "sources__total glue-small-text ng-binding"})
        #table3soup = BeautifulSoup.prettify()  #bs is your BeautifulSoup object
        with open("out.txt","w") as out:
            out.write(table3soup)
    ...
    for x in range(len(mydivsregionssources)):
        ...
        for x in range(len(growthregions)):
            ...
            for x in range(len(mydivsregions)):
                ...
                dictionarytotal = {iteration["name"] : [regionsources, growth, regions]} 
                listtotal.append(dictionarytotal) 
                counter = counter + 1
    for x in range(len(listtotal)):
        print(listtotal[x])

# save into XL
    wb = openpyxl.Workbook() 
    wb.save(filepath) 
    #wb = load_workbook("C:\\Users\\soufi\\Documents\\repositories\\Destination-Insights-with-Google\\google-insights.xlsx")
    for x in listtotal:
        for k,v in x.items():
            key_dict = k 
            value_dict = v 
            ...
            if len(str(k)) < 31:
                ...
                for y in value_dict: 
                    counter2 = 2 
                    ws.cell(row=1, column=1).value = "TOP DEMAND BY SOURCE COUNTRY" 
                    ws.cell(row=1, column=2).value = "Interest" 
                    ws.cell(row=1, column=4).value = "TOP GROWTH BY DESTINATION CITY" 
                    ws.cell(row=1, column=5).value = "Interest" 
                    ws.cell(row=1, column=7).value = "TOP DEMAND BY DESTINATION CITY" 
                    ws.cell(row=1, column=8).value = "Interest" 
                    for z in y: 
